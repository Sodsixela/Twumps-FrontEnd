...
gulp.task("css", css_task({
    src: ["src/app/base.sass"],
    dest: dest_folder + "/app/",
    connect: connect
}));
gulp.task("asset", copy_task({
    src: ["src/asset/**/*.*"],
    dest: dest_folder + "/asset",
    connect: connect
}));
gulp.task("font", copy_task({
...
gulp.task('generate-service-worker', generate_sw_task({
    dest: dest_folder,
    connect: connect
}));
gulp.task('vendors_js', js_vendors_task({
    dest: dest_folder,
    vendors: js_deps,
    connect: connect
}));
